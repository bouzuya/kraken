...
Compiler.prototype._writePostsJson = function() {
  ...
  var data = posts.map(function(post) {
    if (typeof post.minutes === 'undefined') {
      throw new Error(post.date + ' minutes is not defined.');
    }
    return {
...
Compiler.prototype._writeTagsJson = function() {
  ...
  var tagCounts = this._blog.tagCounts();
  var data = Object.keys(tagCounts).reduce(function(tags, tag) {
...
// write <dstDir>/sitemap.xml
Compiler.prototype._writeSitemapXml = function() {
  var dest = Path.join(this._dstDir, 'sitemap.xml');
  var data = [
    '<?xml version="1.0" encoding="utf-8"?>',
    '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">',
  ].concat(this._compiledPosts.map(function(post) {
    return {
