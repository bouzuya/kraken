...
import { listEntryIds as listBbn, parseEntry as parseBbn } from "../parse/bbn";
import {
...
const saveDailyJson = (repository: Repository, outDir: string): void => {
...
const saveMonthlyJson = (repository: Repository, outDir: string): void => {
  repository.getYears().forEach((year) => {
    repository.getMonths(year).forEach((month) => {
      ...
      [`${year}/${month}.json`, `${year}/${month}/index.json`].forEach(
        (file) => {
          writeFile(join(outDir, file), formatted);
        }
      );
...
const saveAtomXml = (repository: Repository, outDir: string): void => {
  const entries = repository.findAll();
  const formatted = formatAtom(entries);
  writeFile(join(outDir, "atom.xml"), formatted);
};

const saveSitemapXml = (repository: Repository, outDir: string): void => {
  const entries = repository.findAll();
...
const saveLinkedJson = (
  ...
  const idString = ({ year, month, date }: EntryId): string =>
    `${year}-${month}-${date}`;
  ...
  entryIds.forEach((entryId, index) => {
    ...
    const sameDays_ =
      typeof sameDays[mmdd] === "undefined"
        ? []
        : sameDays[mmdd].filter((i) => i !== id);
    const formatted = JSON.stringify({
      inbound,
      next,
      outbound,
    ...
    writeFile(
      join(outDir, year, month, date, "related", "index.json"),
      formatted
    );
  });
...
const compileImpl = (
  ...
  ) => Entry,
  {
    ...
    noTokensJson,
    within,
  }: { noIds: boolean; noTokensJson: boolean; within: number }
): Promise<void> => {
  const repository = new Repository(inDir, list, (d, i) =>
    parse(d, i, { noIds })
  );
